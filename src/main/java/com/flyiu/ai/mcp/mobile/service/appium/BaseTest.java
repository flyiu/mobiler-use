...
public class BaseTest {
     ...
     * 获取页面所有可见元素
     */
    protected List<Map<String, Object>> getAllVisibleElements() {
        try {
            // 通过 displayed 获取所有可见元素
            // WebElement element1 = driver.findElement(By.xpath("//*[@text='微信']"));
            // System.out.println("element1: " + element1.getText());
            // System.out.println("element1: " + element1.getAttribute("text"));

            AndroidDriver androidDriver = (AndroidDriver) driver;

            // 获取所有可见元素
            List<WebElement> elements = androidDriver.findElements(By.xpath("//*[@displayed='true']"));
            // 获取所有可见元素的坐标
            List<Map<String, Object>> coordinates = elements.stream().map(element -> {
                Map<String, Object> map = new HashMap<>();
                map.put("text", element.getText());
                map.put("location", element.getLocation());
                map.put("size", element.getSize());
                map.put("resourceId", element.getAttribute("resourceId"));
                return map;
            }).collect(Collectors.toList());
            return coordinates;
        } catch (Exception e) {
            System.err.println("获取页面所有可见元素时出错: " + e.getMessage());
            e.printStackTrace();
        }
        return null;
    }
    ...
    public void openWeChat() {
        try {
            ...
            // 查询页面所有可见元素
            List<Map<String, Object>> elements = getAllVisibleElements();
            System.out.println("页面所有可见元素: " + new Gson().toJson(elements));
     ...
     * @param appActivity
     */
    protected List<Map<String, Object>> getVisibleElementCoordinates() {
        List<Map<String, Object>> elements = getAllVisibleElements();
        return elements.stream().map(element -> {
            Map<String, Object> map = new HashMap<>();
            map.put("text", element.get("text"));
            map.put("center", element.get("center"));
            return map;
        }).collect(Collectors.toList());
    }
